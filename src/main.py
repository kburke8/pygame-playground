...
class Game:
    def __init__(self):
        ...
        # Game state
        self.state = 'character_select'  # 'character_select', 'playing', 'battle', 'countdown', 'rewards', 'inventory'
        self.selected_class = 'warrior'
        self.inventory_page = 0  # 0: skills, 1: passives, 2: gear
        
        # Initialize game world and entities
        self.world = World(WINDOW_WIDTH, WINDOW_HEIGHT)  # Fixed screen size
        self.player = None
        self.enemies = []
        ...
        self.preview_player = Player(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2, self.selected_class)
    ...
    def start_game(self):
        ...
        self.player = Player(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2, self.selected_class)
    ...
    def spawn_enemies(self):
        """Spawn new enemies."""
        self.enemies = []
        enemy_types = ['goblin', 'skeleton', 'slime']
        num_enemies = min(3 + self.player.level // 2, 6)  # More enemies as player levels up
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if event.key == pygame.K_SPACE:
                        self.state = 'playing'
                        self.spawn_enemies()  # Start next round immediately
    ...
    def update(self):
        if self.state in ['playing', 'battle', 'rewards']:
            ...
            self.player.x = self.player.x % WINDOW_WIDTH
            self.player.y = self.player.y % WINDOW_HEIGHT
            
            for enemy in self.enemies:
                enemy.update()
                # Handle screen wrapping for enemies
                enemy.x = enemy.x % WINDOW_WIDTH
                enemy.y = enemy.y % WINDOW_HEIGHT
                
            # Move player towards nearest enemy if in battle
            if self.state == 'battle' and self.enemies:
    ...
    def render(self):
        ...
        if self.state == 'character_select':
            ...
            self.screen.blit(warrior, (WINDOW_WIDTH//2 - warrior.get_width()//2, 200))
            self.screen.blit(rogue, (WINDOW_WIDTH//2 - rogue.get_width()//2, 250))
            self.screen.blit(mage, (WINDOW_WIDTH//2 - mage.get_width()//2, 300))
            ...
            if self.state == 'battle' and self.combat.is_battle_active():
                ...
                self.screen.blit(text, (10, WINDOW_HEIGHT - 50))
            elif self.state == 'inventory':
                ...
                self.screen.blit(exp_text, (WINDOW_WIDTH//2 - exp_text.get_width()//2, 200))
